...
namespace mssql
{
	...
	*/
	...
	Handle<Value> OdbcStatement::get_column_values() const
	{
		...
		{
			arr->Set(c, _resultset->get_column(0, c)->ToValue());
		}
		return result;
	}
	...
	bool OdbcStatement::try_read_row()
	{
		...
		if (_resultset == nullptr) return false;
		if (!_statement) return false;
		const auto& statement = *_statement;
	...
	bool OdbcStatement::get_data_binary(const size_t row_id, const size_t column)
	{
		...
		_resultset->add_column(0, make_shared<BinaryColumn>(column, storage, char_data->size(), reading_column));
		return true;
	}
	...
	bool OdbcStatement::d_binary(const size_t row_id, const size_t column)
	{
		if (_prepared)
		{
			...
			auto amount = ind[0];
			_resultset->add_column(0, make_shared<BinaryColumn>(column, storage, amount, more));
			return true;
		}
	...
	bool OdbcStatement::try_read_columns()
	{
		...
		for (auto c = 0; c < column_count; ++c) {
			...
			res = dispatch(definition.dataType, 0, c);
			if (!res) {
				break;
			}
		}
	...
	bool OdbcStatement::lob(SQLLEN display_size, const int column)
	{
		...
		_resultset->add_column(0, make_shared<StringColumn>(column, storage, uint16_data->size(), reading_column));
		return true;
	}
